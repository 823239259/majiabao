  ...
  <div>
    <template v-if="keyWord">
      <ul class="result-list" v-if="result_list.length">     
        <li v-for="item in result_list" class="checked" @click="ToItem(item.id)">{{ item.contractName }} {{ item.code }}</li>
      </ul>
...
.result-list{
  ...
  li{
    ...
    &::before{
      ...
      background: url('../../assets/images/contract_search/search_fff.png') center center no-repeat;
      background-size: 100%; 
    }
  }
  .checked{
