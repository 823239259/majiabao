		...
		drawkline: function(state, x) {
			...
			if(state.isshow.isklineshow == false) {
				...
				if(document.getElementById(x.id1) != null){
					var ase = document.getElementById(x.id1);
...
//					name: '成交量',
		...
		describeItem(state, orderTemplist) {
			const myOrder = ['CL', 'BRN', 'QM'];
			...
			myOrder.forEach(item => {
				...
				let sendMsg = {
				  Method: 'Subscribe',
				  Parameters: {
				    ExchangeNo: currentItem.ExchangeNo,
				    CommodityNo: currentItem.CommodityNo,
				    ContractNo: currentItem.MainContract
				  }
				}
				state.market.myOrderList.push(currentItem)
				state.quoteSocket.send(JSON.stringify(sendMsg));
			})
